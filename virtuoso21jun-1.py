...
def fetch_journey_details(job_id, api_token):
    base_url = f"https://api.virtuoso.qa/api/testsuites/latest_status"
    params = {
        "jobId": job_id,
        "includeSequencesDetails": "true",
        ...
        if response.status_code == 200:
            ...
            for key, value in data.items():
                if 'journey' in value and 'title' in value['journey']:
                    ...
                    print("outcome ", outcome," ",value['journey']['title'])
                    if outcome == 'PASS':
                        journey_names['passed'].append(value['journey']['title'])
                    elif outcome == 'FAIL' or outcome == 'ERROR':
...
def total_TC_executed_passed(project_id, plan_id, api_token, next_execution_time, team_name):
    ...
    for goal_id, job_data in filtered_jobs.items():
        ...
        # passed_journey_names = []
        # failed_journey_names = []
        journey_names = fetch_journey_details(job_data['job_id'], api_token)
        # if 'lastExecution' in journey_details and 'statistics' in journey_details['lastExecution']:
        #     for journey_id, journey_data in journey_details.items():
        #         journey_name = journey_data.get('journey', {}).get('title', f"Journey {journey_id}")
        #         outcome = journey_data['lastExecution']['statistics'].get('outcome')
        #         if outcome == "PASS":
        #             passed_journey_names.append(journey_name)
        #         elif outcome in ["FAIL", "ERROR"]:
        #             failed_journey_names.append(journey_name)
        # for key, value in journey_details.items():
        #     if 'journey' in value and 'title' in value['journey']:
        #         outcome = value.get('lastExecution', {}).get('statistics', {}).get('outcome', '')
        #         print("outcome ", outcome," ",value['journey']['title'])
        #         if outcome == 'PASS':
        #             passed_journey_names.append(value['journey']['title'])
        #         elif outcome == 'FAIL' or outcome == 'ERROR':
        #             failed_journey_names.append(value['journey']['title'])
        ...
        for project in team['projects']:
            ...
            for planner_id in project['planner_id']:
                ...
                if next_execution_time:
                    executed, passed, failed, details = total_TC_executed_passed(
                        project['project_id'], planner_id, api_token, next_execution_time, team['team_name']
                    )
...
def export_summary_csv(team_data, summary_file, detailed_summary_file, detailed_summary):
    ...
    with open(detailed_summary_file, 'w', newline='') as csvfile:
        fieldnames = ['Team Name', 'Execution Planner Name', 'Goal Name', 'Total TC Executed', 'Total TC Passed', 'Total Failed', 'Passed Journey Names', 'Failed Journey Names']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        writer.writeheader()
        for team_name, planners in detailed_summary.items():
            for planner_name, goals_data in planners.items():
                for goal_name, tc_data in goals_data.items():
                    writer.writerow({
                        'Team Name': team_name,
                        'Execution Planner Name': planner_name,
                        'Goal Name': goal_name,
                        'Total TC Executed': tc_data['TC Executed'],
                        'Total TC Passed': tc_data['TC Passed'],
                        'Total Failed': tc_data['TC Failed'],
                        "Passed Journey Names": "; ".join(tc_data["details"]["passed_journeys"]),
                        "Failed Journey Names": "; ".join(tc_data["details"]["failed_journeys"])
                    })
...
if __name__ == "__main__":
    ...
    try:
        ...
        summary_file = f'log_{formatted_time_ist}.csv'
        detailed_summary_file = f'detailed log_{formatted_time_ist}.csv'
        export_summary_csv(team_data, summary_file, detailed_summary_file, detailed_summary)
        print("Output saved to summary1.csv and detailed_summary1.csv")
